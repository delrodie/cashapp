...
#[AsCommand(
    name: 'app:import-default-db',
    description: 'Importation de la base de données par defaut',
)]
class ImportDefaultDbCommand extends Command
{
    private $startTime;
    private $endTime;
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $defaultDatabaseUrl = $this->params->get('DATABASE_URL');

        // Extraction du nom de la base de données via ARCHIVE_DATABASE_URL
        preg_match('/\/([^\/?]+)\?/', $defaultDatabaseUrl, $matches);
        $databaseName = $matches[1];

        // Importation de la base de données
        $importCommand = sprintf(
            'mysql -h %s -u %s -p%s %s < public/.sql/cashapp_nouveau.sql',
            $this->parseDatabaseHost($defaultDatabaseUrl),
            $this->parseDatabaseUser($defaultDatabaseUrl),
            $this->parseDatabasePassword($defaultDatabaseUrl),
            $databaseName
        );
        ...
        $progressBar->finish();
        $io->success('Base de données importée avec succès!');
